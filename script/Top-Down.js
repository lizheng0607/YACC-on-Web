...
var recursiveVn=[20];//直接递归元素集合
var recursiveVn1;//间接左递归元素
var str1,str2,str3;//临时字符串
var LLflag=0;
var LLpanduan=0;
var myindex=-10;//间接左递归标记
var myi=0;
///////////
var I;
var kk=0;
...
function myCheck()//间接左递归中，整理文法 同时输出
{
    var last=0;
    while(str[last]!=""&&str[last]!=undefined)
    {
        last++;
    }
    last--;
    ...
    ////////显示暂时修改后的输入框文法
                        ...
                        while(str[i]!="")
                        {
                            if(mystr2=="")mystr2=str[i];
                            else{
                                mystr2+='\n'+str[i];
                                                        }
                            i++;
                        }
                        document.getElementById("t_left").value=mystr2;//显示消除左递归后的文法

}
function FFVN()//消除间接左递归 [S,A,B,&]
{   if(myindex==-10)
    {
         
        for(var i=0;i<Vnnum;i++)
        {
            myindex=FindX(recursiveVn1[i],"&",10)-1;//获取B的index
   ...
   //EVN[index+1]="";
   if(myindex>=1){
        var myKey=EVN[myindex-1];
        var myKey1=EVN[myindex];
        var i=myi;
        while(str[i]!=""&&str[i]!=undefined)
        {
            if(str[i].substring(0,str[i].indexOf("-"))==myKey)
            {   mystr+=str[i]+'\n';
                var mystrxx=str[i].substring(0,str[i].indexOf(myKey1));
                var mystrxx1=str[i].substring(str[i].indexOf(myKey1)+1,str[i].length);
                var j=0;
                while(str[j]!=""&&str[j]!=undefined)
                {
                    if(str[j].substring(0,str[j].indexOf("-"))==myKey1)
                    {       mystr+=str[j]+'\n';
                            var myFinalstr=mystrxx+str[j].substring(str[j].indexOf(">")+1,str[j].length)+mystrxx1;
                            mystr+="合并结果为："+myFinalstr+'\n';
                            addL(str,myFinalstr);
                            str[j]="&";
                ...
                if((str[j]==""||str[j]==undefined)&&j!=100)
                {
                    str[i]="&"; 
                    myCheck();//暂时整理输入的文法
                    myindex--;
       ...
       flag=31;//接下来 进行消除直接左递归
       str1+="接下来，消除文法的直接左递归"+'\n';
       str2=str1;
       document.getElementById("t_below_right1").innerHTML=str2;
       findSL();
   }
   

}
function jianjieEE(A,n)
{   
    if(recursiveVn1[n][0]=="")
    recursiveVn1[n][0]=A;
    ...
    while(str[i]!=""&&str[i]!=undefined)
    {       
         if(str[i].substring(0,str[i].indexOf('-'))==A)
        {   index=str[i].indexOf(">");
            if(str[i].charAt(index+1)>='A'&&str[i].charAt(index+1)<='Z'&&str[i].substring(0,str[i].indexOf('-'))!=str[i].charAt(index+1))
            {   if(recursiveVn1[n][0]!=str[i].charAt(index+1))
                    {    addL(recursiveVn1[n],str[i].charAt(index+1));
                        var lz=jianjieEE(str[i].charAt(index+1),n);
                        if(lz==0)return 0;
                        else if(lz==-1) ;//return -1;
                    }
                else{
                    addL(recursiveVn1[n],"&");
                    alert("此文法具有间接左递归！");
...
{              
    if(recursiveVn[counter1]!="")
                    { for(var i=0;i<str.length;i++)
                        ...
                        if(str[i].charAt(0)==recursiveVn[counter1]&&str[i].charAt(0)==str[i].charAt(3))
                            {
                                ...
                                str2+=str[i]+'\n';
                                document.getElementById("t_below_right1").innerHTML=str2;
                                //document.getElementById("t_below_right2").innerHTML=str[i];
                        ...
                        document.getElementById("t_below_right1").innerHTML=str2+"已消除文法左递归";
                        //str1="";
                        alert("已消除左递归");
                        //flag=0;
...
{    //document.getElementById('t_below_left').innerHTML="<span style='font-size:12px; color:green;'>This is content</span>"
    ...
    traverse();
     ...
     recursiveVn1=new Array();//
                    for(var i=0;i<Vnnum;i++)
                        {recursiveVn1[i]=new Array();
                         for(var j=0;j<10;j++)
                            {
                                recursiveVn1[i][j]="";
                            }
...
function myKeydown(e)//键盘事件
{
    var e = event||window.event||arguments.callee.caller.arguments[0];
             if(e && e.keyCode==118){ // 按 F7
                if(flag==31)//31表示消除直接左递归
                {     eliminateRecursive();
                     scrollButtom();
                }
                else if(flag==32)//32表示消除间接左递归
                {
                     //if(flag==32)
                     var i=0;
                     var myh=-1;
